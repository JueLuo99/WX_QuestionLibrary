...
Page({
  ...
  onLoad: function (options) {
    this.setData({allCorrect:options.allCorrect,allTotal:options.allTotal,tikuName: options.tikuName})
    this.get7dayInfo()
  },
    ...
    db.collection("AnswerRecords").orderBy("date",'desc').limit(7).get().then(res=>{
      ...
      for(var i=0;i<_7.length;i++){
        ...
        if(_7[i].date.getFullYear() == today.getFullYear()&&_7[i].date.getDate() == today.getDate()&&_7[i].date.getMonth() == today.getMonth()){
          this.setData({todayCorrect: _7[i].correct,todayTotal: _7[i].total})
        }
      }
      var maxHeight = 200;
      for(var i=0;i<_7.length;i++){

...
Page({
  ...
  getQuestions: function () {
    ...
    this.data.qs = [];
    // 匹配对应的题库并从中抽取题目
    db.collection(this.data.tikuName).aggregate().match({
      type: "s"
    }).sample({
      size: this.data.num_of_s
    }).end().then(res => {
      ...
      for (var i = 0; i < res.list.length; i++) {
        ...
        while (tItem["choices"].length > 0) {
          ...
          choices.push({
            "answers": tItem["choices"][o],
            "isTrue": tItem["answers"].indexOf(tItem["choices"][o]) > -1,
            "class": "choice",
            "selected": false
          })

...
Page({
    ...
    wx.cloud.callFunction({
      ...
      success: res => {
        ...
        for (var i = 0; i < res.result.data[0]["choices"].length; i++) {
          tmpChoices.push({
            "answers": res.result.data[0]["choices"][i],
            "isTrue": res.result.data[0]["answers"].indexOf(res.result.data[0]["choices"][i]) > -1,
            "class": "choice",
            "selected": false
          });
        }
  ...
  syncAnswerRecord: function () {
    const db = wx.cloud.database()
    db.collection("ExerciseAnswerRecords").where({
      tiku: this.data.tikuName
    }).get().then(res => {
      console.log("ExerciseAnswerRecords", res)
      if (res.data.length == 0) {
        db.collection("ExerciseAnswerRecords").add({
          data: {
            tiku: this.data.tikuName,
            ids: []
          }
  ...
  addWrongQuestion: function () {
    const db = wx.cloud.database()
    db.collection("WrongQuestions").where({
      id: this.data.questionList[0]._id
    }).count().then(res => {
      if (res.total == 0) {
        db.collection("WrongQuestions").add({
          data: {
            tikuName: this.data.tikuName,
            id: this.data.questionList[0]._id
          }
